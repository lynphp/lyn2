...
class View
{
    public static function render_error($error, $responseCode = 404):string
    {
    ...
    3: "app.shoe"
    */
    private static function get_component_attributes($attributes):array
    {
    ...
    private static function get_attribute_by_key($attr):array
    {
        parse_str($attr, $vars);
        return $vars;
    }
    private static function get_web_component_element_tags($html):array
    {
        ...
        if ($file === $callerFile) {
            ...
            if (str_contains($output, "lyn-style") && strlen($css) < 5) {
                ...
                if (!file_exists(public_server_path . '/css/' . $css_hash_file)) {
                    ...
                    $css_file = fopen(public_server_path . '/css/' . $css_hash_file, "w");
                    fwrite($css_file, $css_content);
                    fclose($css_file);
                    $css_hash_file = $css_hash_file;
                }
